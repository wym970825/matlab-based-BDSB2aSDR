classdef scint_calculator < handle
    ...
    methods (Static)
        ...
        function kfR = computeKF_R(scintRes, settings, SM)
            ...
            %           / (CN0*(1 - S4^2))                         [rad^2]
            %
            % S4 is corrected by sqrt(max(S4_ori^2 - S4_corr^2, 0)).  The
            % formula is valid below S4 < 0.707, so S4 is capped below the
            % singularity.  R_omega is propagated from differenced phase noise.
            %
            % Optional settings.KF fields:
            %   conkerEqPllBn_Hz   equivalent Bn for Rphi
            %   conkerS4Max        S4 cap, default 0.70, max 0.706
            %   RphiCeil_deg       upper limit for Rphi, default 60 deg
            %   RomegaCeil_Hz      upper limit for Romega, default 200 Hz
            ...
            kfR.Rphi   = (5 * pi / 180)^2;
            kfR.Romega = (2 * pi * 3)^2;
            kfR.model  = 'ConkerEq5_ampOnly';

            if nargin < 1 || ~isstruct(scintRes) || ...
                    ~isfield(scintRes,'valid') || ~scintRes.valid
                return;
            end
            if ~isfield(scintRes,'S4_ori') || ~isfield(scintRes,'S4_corr') || ...
                    ~isfinite(scintRes.S4_ori) || ~isfinite(scintRes.S4_corr)
                return;
            end

            % Correct S4 for receiver thermal noise.
            S4_raw = sqrt(max(scintRes.S4_ori^2 - scintRes.S4_corr^2, 0));
            if ~isfinite(S4_raw), return; end

            S4max = 0.70;
            if nargin >= 2 && isstruct(settings) && isfield(settings,'KF') && ...
                    isstruct(settings.KF) && isfield(settings.KF,'conkerS4Max') && ...
                    ~isempty(settings.KF.conkerS4Max)
                S4max = settings.KF.conkerS4Max;
            end
            S4max = min(max(S4max, 0), 0.706);
            S4 = min(S4_raw, S4max);
            kfR.S4_raw = S4_raw;
            kfR.S4     = S4;

            % CN0 and coherent integration time.
            ...
            Tcoh = 1e-3;
            if isfield(scintRes,'allLong') && scintRes.allLong && ...
                    nargin >= 2 && isstruct(settings) && isfield(settings,'longCoh_ms') && ...
                    ~isempty(settings.longCoh_ms)
                Tcoh = settings.longCoh_ms * 1e-3;
            elseif nargin >= 2 && isstruct(settings) && isfield(settings,'intTime') && ...
                    ~isempty(settings.intTime)
                Tcoh = settings.intTime;
            end
            Tcoh = max(Tcoh, eps);
            ...
            denomAmp  = max(1 - S4^2, eps);
            denomFade = max(1 - 2*S4^2, eps);
            Rphi = Bn * (1 + 1/(2*Tcoh*CN0_lin*denomFade)) / (CN0_lin*denomAmp);

            RphiFloor = (3 * pi / 180)^2;
            if nargin >= 2 && isstruct(settings) && isfield(settings,'KF') && ...
                    isstruct(settings.KF) && isfield(settings.KF,'RphiFloor_deg') && ...
                    ~isempty(settings.KF.RphiFloor_deg)
            ...
            Tfll = 1e-3;
            if nargin >= 2 && isstruct(settings) && isfield(settings,'intTime') && ...
                    ~isempty(settings.intTime)
                Tfll = settings.intTime;
            end
            Tfll = max(Tfll, eps);
            Romega = 2 * Rphi / (Tfll^2);

            if nargin >= 2 && isstruct(settings) && isfield(settings,'KF') && ...
                    isstruct(settings.KF) && isfield(settings.KF,'RomegaFloor_Hz') && ...
                    ~isempty(settings.KF.RomegaFloor_Hz)
                RomegaFloor = (2 * pi * settings.KF.RomegaFloor_Hz)^2;
            else
                RomegaFloor = (2 * pi * 3)^2;
            end
            RomegaCeil = (2 * pi * 200)^2;
            if nargin >= 2 && isstruct(settings) && isfield(settings,'KF') && ...
                    isstruct(settings.KF) && isfield(settings.KF,'RomegaCeil_Hz') && ...
                    ~isempty(settings.KF.RomegaCeil_Hz)
                RomegaCeil = (2 * pi * settings.KF.RomegaCeil_Hz)^2;
            end
            Romega = min(max(Romega, RomegaFloor), RomegaCeil);

            kfR.Rphi   = Rphi;
            kfR.Romega = Romega;
            kfR.valid  = isfinite(Rphi) && isfinite(Romega) && Rphi > 0 && Romega > 0;
        end
